(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj = new Static("Static11","80","529","840","30",null,null,null,null,null,null,this);
            obj.set_taborder("23");
            obj.set_text("모달 팝업창 오픈, 파라메터 전송. 팝업창에서 파라메터 받기. / 3 - 2 모달 팝업창에서 부모창에 리턴값 전송. 부모창에서 리턴값 받기");
            this.addChild(obj.name, obj);

            obj = new Static("Static12","80","569","770","30",null,null,null,null,null,null,this);
            obj.set_taborder("24");
            obj.set_text("모달리스 팝업창 오픈, 파라메터 전송.  / 3 – 4 모달리스 팝업창에서 부모창에 ds_child 데이터셋을 전송.");
            this.addChild(obj.name, obj);
            ...
            obj = new Button("btn00","398","108","75","40",null,null,null,null,null,null,this);
            obj.set_taborder("26");
            obj.set_text("btn00");
            this.addChild(obj.name, obj);

            obj = new Static("staTimer","340","192","156","30",null,null,null,null,null,null,this);
            obj.set_taborder("27");
            obj.set_text("sta00");
            this.addChild(obj.name, obj);
            // Layout Functions
        ...
        this.btn_Exe1_1_onclick = function(obj,e)
        {
        	...
        	//invisible 오브젝트만 보여줌
        // 	var arrObj = this.objects;
        //
        // 	for(var i=0; i<arrObj.length; i++)
        // 	{
        // 		trace(arrObj[i] + " : " + arrObj[i].id); //로그로 보여줌
        // 	}

        	this.fn_compList(this);
        };

        //컴포넌트 정보 가져오는 방법 이걸 써라(오브젝트/컴포넌트 정보를 구하기. )
...

        {
        	...
        	for(var i=0; i<arrObj.length; i++)
        	{
        		...
        		if(sType == "[object Div]") {
        			this.fn_compList(arrObj[i].form); //div 계속 돈다
        		}
        		else if(sType == "[object Tab]") { //tap 안에서 계속 돈다
        ...
        //컴포넌트ID String 값을 오브젝트로 만들기
        this.btn_Exe1_2_onclick = function(obj,e)
        {
        		var sGridId = this.Grid3.name;
        		trace(sGridId + " : " + typeof sGridId);

        		//var objGrid = eval("this." + sGridId); //eval 은 쓰지말기
        		var objGrid = this.all[sGridId]; //components - visible 컴포넌트를 나타댐
        		trace(objGrid + " : " + typeof objGrid);
        };

        //타이머를 이용하여 시간 표현
        this.btn_Exe2_1_onclick = function(obj,e)
        {							   	// 123 = id 값을 준거다 setTimer 를 쓸땐 id 를 줘야됨
        		this.setTimer(123,1000); // 1000 = 1초 10000 = 10초
        };

        this.Form_ontimer = function(obj,e)
        {
        	if(e.timerid == 123) //id가 맞으면 fn_getTime 펑션 시행
        	{
        		this.fn_getTime();
        		//this.killTimer(123); 한번만 실행하고 싶으면 이거 넣으면 됨
        	}
        };
        ...
        this.fn_getTime = function()
        {
        	...
        	var nH = objDate.getHours(); //0시부터 24 로 표현
        	var nM = objDate.getMinutes();
        	var nS = objDate.getSeconds();

        	var sH = (nH.toString().length <2 ? "0" +nh.toString() : nH.toString()); //toString 으로 바꿔야 연산이 안되서 쓴다
        	var sM = (nM.toString().length <2 ? "0" +nh.toString() : nM.toString());
        	var sS = (nS.toString().length <2 ? "0" +nh.toString() : nS.toString());

        	//시계 표시하게 해주는 것
        	this.staTimer.set_text(sH + " : " + sM + " : " + sS); //static 컴포넌트 하나 만들고 속성에 id를 staTimer 지정후 시간 돌리면 시계가 보인다.
        	trace(sH + " : " + sM + " : " + sS); //로그에 시간이 보이도록
        }
        ...
        //모달 - 부모 폼에 종속 (속도가 빨라 권장)
        //모달리스 - 부모 폼에 종속 되지 않음 (속도가 느려 권장 X )
        ...
        this.btn_Exe3_1_onclick = function(obj,e)
        {
        	...
        	objChildFrame.init("chf_popup1" //차일드 팝업 이름
        		...
        		, null
        		, null
        		, "EduBasic::Exe/Exe_Chap4_Form_Popup.xfdl"); //띄우려고 하는 팝업 링크
        ...
        this.btn_Exe3_3_onclick = function(obj,e)
        {
        	...
        	//좌표를 구하는 로직 기준이 없기때문에 수동으로 좌표를 정해 줘야 한다.

...
//	String filePath = request.getRealPath("./" + folderName) + "/";
	String url = request.getSession().getServletContext().getRealPath("/edupack_jsp/") + "\\" + folderName;	
	String filePath = url;	
	...
	ds.addColumn("FILE_PATH", DataTypes.STRING, 255);
	...
	if(rfiles != null)
	{
		for(int i=0; i<rfiles.length; i++)
		{
			if(rfiles[i].isFile())
			{
				...
				ds.set(row, "FILE_PATH", url + "\\" + rfiles[i].getName());

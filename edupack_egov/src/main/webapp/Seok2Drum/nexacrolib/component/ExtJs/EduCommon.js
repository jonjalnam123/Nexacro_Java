...
{
	var compID = this.ufnGetCompId(obj);	
	if( this.ufnIsNull( this.components[compID] ) ){
...
pForm.cfnSetMultiCombo = function(obj)
{
	...
	if(this.ufnIsNull(nDisplayCnt))	nDisplayCnt = 10;
	
	if(this.ufnIsNull(sInnerDataset) || this.ufnIsNull(sCodeColumn)	|| this.ufnIsNull(sDataColumn)){
	...
	if(this.ufnIsNull(objInnerDataset)){
		alert("InnerDataset Object Error");
		return;
	}
	
	var sCompFull = this.ufnGetCompId(obj);	// formid.div00.form.combo00
	...
	if(this.ufnIsNull(objGrid)){
		...
		objGrid.addEventHandler("onheadclick", this._ufnHeadCheckSelectAll, this);
...
pForm._setComboText = function(uParam, arrTextList, arrValueList)
{
	...
	if(this.ufnIsNull(sComboText))	uParam.combo.set_text("멀티선택");

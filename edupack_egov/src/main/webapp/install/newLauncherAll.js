...
if (is64bit) {
    ...
    if (bXplatform) {
        ...
        if (bUse64bit) {
            strEnginesetupkey = strEnginesetupkey64;  //64bit 엔진 key
        } else {
            strEnginesetupkey = strEnginesetupkey32; //32bit 엔진 key
        }
    }
    ...
    if (type == "newlanucher") {
        ...
        if (bXplatform) {
            ...
            if (bJasmine) {
                if (is64bit) {
                    if (bUse64bit) {
                        strSetupRuntimeName = "nexacro17_SetupRuntime_x64.exe";
                    } else {
                        strSetupRuntimeName = "nexacro17_SetupRuntime_x86.exe";
                    }
                } else {
                    strSetupRuntimeName = "nexacro17_SetupRuntime_x86.exe";
                }
                var downURL = strDownloadPath + "/17.1/" + strSetupRuntimeName;
                istalledMsg("EngineDownload", contents, downURL);
                return downURL;
            } else {
                ...
                if (userAgent.indexOf("Windows NT 5.1") != -1 ||
                    ...
                    strSetupRuntimeName = "nexacro17_SetupRuntime_XP.exe";
                } else {
                    if (is64bit) {
                ...
                var downURL = strDownloadPath + "/17/" + strSetupRuntimeName;
                istalledMsg("EngineDownload", contents, downURL);
                return downURL;
            }
        }
        ...
        if (action_sub == 'basic') {
            ...
            if (bXplatform) {
                ...
                objProduct.value = {
                    ...
                    "onlyone"       : bOnlyone,
                    "splashmessage" : 'none',
                    "setupresource" : bSetupResource,   //bSetupResource 항목 추가.
                    "enginesetupkey": strEnginesetupkey
                    //"screenid"      : ""              //null이면 TPLSvc 서비스가 중단될수 있음. empty값 또는 미설정(2019.05.03)
                };
            }
